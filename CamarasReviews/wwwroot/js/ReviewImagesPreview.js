function createImagePreview(containerId, inputId, previewId) {
    ...
    const multiSelectorUniqPreview = document.querySelector(`#${containerId} #${previewId}`);

    function getIndexOfFileList(name, list) {
        return list.reduce((position, file, index) => (file.name === name ? index : position), -1);
    }
    ...
    async function getUniqFiles(newFiles, currentListFiles) {
        return new Promise((resolve) => {
            Promise.all(newFiles.map((inputFile) => encodeFileToText(inputFile))).then((inputFilesText) => {
                Promise.all(currentListFiles.map((savedFile) => encodeFileToText(savedFile))).then(
                    (savedFilesText) => {
    ...
    function eventDragOver(event) {
        ...
        multiSelectorUniqPreview.querySelectorAll("li").forEach((item) => item.classList.remove(classDragOver));
...
createImagePreview("MultiProductImages", "productFiles", "ProductImagesPreview");
